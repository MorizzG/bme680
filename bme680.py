...
class BME680:
    def __init__(self, i2c_addr=constants.I2C_ADDR_PRIMARY):
    ...
    def soft_reset(self):
        ...
        self.i2c.write(constants.SOFT_RESET_ADDR, constants.SOFT_RESET_CMD)
    ...
    def set_humidity_oversample(self, value):
        ...
        self.i2c.write_bits(value, constants.CONF_OS_H_ADDR, constants.OSH_POS, constants.OSH_MSK)
    ...
    def select_gas_heater_profile(self, value):
        ...
        if value > constants.NBCONV_MAX or value < constants.NBCONV_MIN:
            raise ValueError(
                f"Profile '{value}' should be between {constants.NBCONV_MIN} and {constants.NBCONV_MAX}"
            )

        self.gas_settings.nb_conv = value
        self.i2c.write_bits(
            value, constants.CONF_ODR_RUN_GAS_NBC_ADDR, constants.NBCONV_POS, constants.NBCONV_MSK
        )
    ...
    def set_gas_heater_temperature(self, value, nb_profile=0):
        ...
        if nb_profile > constants.NBCONV_MAX or value < constants.NBCONV_MIN:
            raise ValueError(
                f'Profile "{nb_profile}" should be between {constants.NBCONV_MIN} and {constants.NBCONV_MAX}'
            )

        self.gas_settings.heatr_temp = value
        temp = int(self._calc_heater_resistance(self.gas_settings.heatr_temp))
        self.i2c.write_byte(temp, constants.RES_HEAT0_ADDR + nb_profile)
        ...
        if nb_profile > constants.NBCONV_MAX or value < constants.NBCONV_MIN:
            raise ValueError(
                'Profile "{}" should be between {} and {}'.format(
                    nb_profile, constants.NBCONV_MIN, constants.NBCONV_MAX
                )
            )
    ...
    def get_power_mode(self):
        """Get power mode."""
        self.power_mode = self.i2c.read_byte(constants.CONF_T_P_MODE_ADDR) & constants.MODE_MSK
        return self.power_mode

    def set_power_mode(self, mode, blocking=True):
        """Set power mode"""
        if mode not in (constants.SLEEP_MODE, constants.FORCED_MODE):
            raise ValueError("Power mode has to be SLEEP_MODE or FORCED_MODE")

        self.power_mode = mode

        self.i2c.write_bits(
            mode, constants.CONF_T_P_MODE_ADDR, constants.MODE_POS, constants.MODE_MSK
        )
    ...
    def _calc_pressure(self, pressure_adc):
        ...
        var1 = (
            (((var1 >> 2) * (var1 >> 2)) >> 13) * ((self.calibration_data.par_p3 << 5)) >> 3
        ...
        var1 = (
            self.calibration_data.par_p9 * (((calc_pressure >> 3) * (calc_pressure >> 3)) >> 13)
        ) >> 12
    ...
    def _calc_heater_resistance(self, temperature):
        ...
        var2 = (self.calibration_data.par_gh1 + 784) * (
            ((((self.calibration_data.par_gh2 + 154009) * temperature * 5) / 100) + 3276800) / 10
        )
        var3 = var1 + (var2 / 2)
